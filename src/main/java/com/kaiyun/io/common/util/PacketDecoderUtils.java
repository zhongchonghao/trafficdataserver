...
public class PacketDecoderUtils {
    private static Logger logger = LoggerFactory.getLogger(PacketDecoderUtils.class);
    ...
    public static byte[] decoderEscape(ByteBuf buf) {
        ...
        logger.info("原始报文：{}", bytes2HexStr(originalPacket));
        logger.info("转义后的报文：{}", bytes2HexStr(correctPacket));
        return correctPacket;
    }

    private static byte[] decoderEscape(byte[] originalPacket) {
     ...
     * @param packet
     */
    public static ByteBuf baseDecoder(byte[] bytes, BasePacket packet) throws Exception {
